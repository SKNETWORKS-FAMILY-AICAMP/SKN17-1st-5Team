...
sql = "SELECT type, total FROM registed_car WHERE city=%s AND town=%s AND added_year=%s AND added_month=%s AND local_regist='Y'"
cursor.execute(sql, (city, town, year, month))
result_rows = cursor.fetchall()

if result_rows:
    df = pd.DataFrame(result_rows, columns=['Type', 'Registered Cars'])

    st.dataframe(df)
    
    st.write(f"{city} {town} - 차량 유형별 등록 현황")

    fig, ax = plt.subplots(figsize=(6, 6))  # 사이즈 키워서 겹침 방지
    ...
    wedges, texts, autotexts = ax.pie(
        ...
        autopct='%1.1f%%',
        startangle=140,
        labeldistance=1.15,       # 라벨을 원 밖으로 배치
        pctdistance=0.75,         # 퍼센트는 원 안쪽으로
        wedgeprops=dict(width=0.6)  # 도넛형으로 약간 파내기
    )
    ...
    st.write("선택한 지역의 차량 등록 정보가 없습니다.")
